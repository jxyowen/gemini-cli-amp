...
5. **Debug Verification:** Follow this detailed debugging process to verify API parameter flow:
   ...
   3. **Start Application with Log Redirection:** Start the application and redirect all output to amp-cli-debug/amp-cli-debug.log, such as \`mvn spring-boot:run > amp-cli-debug/amp-cli-debug.log 2>&1 &\`. Use tail to observe the logs and monitor whether the application starts successfully and identify the application port number. Stop observing after successful startup.
