...
2. **Backend Code Generation:** Based on API parameter mapping (backendName and backendService.service, backendService.method, and backendService.paramTypes and backendService.url), generate backend code with clearly specified file paths. When generating code involving imports, annotations (@), and utility classes, first reference existing code implementations in the project rather than generating directly.
3. **Debug Verification:** Follow this detailed debugging process to verify API parameter flow:
   ...
   3. **Check and Clear Existing Processes:** Before starting the application, use \`lsof -i :8080\` (replace 8080 with the actual port) to check if there are any existing processes occupying the port. If processes are found, use \`kill <PID>\` to terminate them to avoid port conflicts during application startup.
   4. **Start Application with Log Redirection:** Start the application and set up two output redirections: one redirect all output to amp-cli-debug/app.log for application startup information, and another redirect debug information using filter condition \`grep --line-buffered 'amp-cli-debug:'\` to amp-cli-debug/amp-cli-debug.log, such as \`mvn spring-boot:run 2>&1 | tee amp-cli-debug/app.log | grep --line-buffered 'amp-cli-debug:' > amp-cli-debug/amp-cli-debug.log &\`. 
   5. **Check Application Startup Status:** Use \`tail -n100\` multiple times to observe the application startup information to monitor whether the application starts successfully and identify the application port number. Check for successful startup indicators in the logs (such as "Started Application" or "Tomcat started on port"). If startup is successful, proceed to step 6. If startup fails, stop observing and adjust the code to fix the startup issues before retrying.
   6. **Health Check Verification:** Wait 10 seconds after confirming successful startup from logs, then check if the health check endpoint (such as http://127.0.0.1:8080/check_health) returns success to ensure the application is running properly. If it fails, repeat step 6. If it still fails after 60 attempts, ask the user if there are any exceptions. If you need to kill the application, first use commands like \`lsof -i :8080\` to query the process ID (PID) occupying the port, then use \`kill <PID>\` to terminate the process.
   7. **Call API Debug Tool:** Use API debugging tools to obtain gateway-side input and output parameter information.
